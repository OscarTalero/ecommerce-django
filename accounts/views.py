...
from carsh.views import _cart_id
from carsh.models import Cart, CartItem
import requests
...
def activate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        user.is_active = True
        user.save()
        messages.success(request, 'Conglaturations, your account is active')
        return redirect('login')
    else:
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request, 'Succees password reset')
            return redirect('resetPassword')
        else:
            messages.error(request, 'Confirmed Password Not Match')
            return redirect('login')
    else:
